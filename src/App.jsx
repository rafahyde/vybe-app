...
function isOpenNow(hours) {
  ...
  try {
    ...
    const todayKey = Object.keys(hours).find(k => k.split(",").some(d => d.trim() === currentDay) || k.includes(currentDay));
    if (!todayKey) return null;
    if (hours[todayKey] === "fechado") return false;
    const parts = hours[todayKey].split("-");
    if (parts.length < 2) return null;
    const [oh, om] = parts[0].split(":").map(Number);
    const [ch, cm] = parts[1].split(":").map(Number);
    const openTime = oh * 100 + (om || 0);
    const closeTime = ch * 100 + (cm || 0);
    if (closeTime < openTime) return hhmm >= openTime || hhmm <= closeTime;
...
function normalizePlace(p) {
  if (!p) return null;
  const hours = p.hours && typeof p.hours === "object" ? p.hours : null;
  return {
    id: p.id, name: p.name || "Sem nome", type: p.type || "bar",
    distance: p.distance || "", tags: Array.isArray(p.tags) ? p.tags : [],
    rating: Number(p.rating) || 0, crowd: Number(p.crowd) || 0,
    music: p.music || "", cover: p.cover || "Consultar", color: p.color || "#A78BFA",
    lat: Number(p.lat) || 0, lng: Number(p.lng) || 0, checkins: Number(p.checkins) || 0,
    address: p.address || "", reports: Array.isArray(p.reports) ? p.reports : [],
    image_url: p.image_url || null, hours, open_now: hours ? isOpenNow(hours) : null,
  };
}
...
function normalizeEvent(e) {
  if (!e) return null;
  return {
    id: e.id, name: e.name || "Sem nome", type: e.type || "Outros",
    date: e.date || "", time: e.time || "", starts_at: e.starts_at || null,
    location: e.location || e.venue_name || "", description: e.description || "",
    lineup: Array.isArray(e.lineup) ? e.lineup : [], confirmed: Number(e.confirmed) || 0,
    color: e.color || "#A78BFA",
    gradient: e.gradient || `linear-gradient(135deg, ${e.color || "#A78BFA"}, #60A5FA)`,
    emoji: e.emoji || "🎉", tickets: Array.isArray(e.tickets) ? e.tickets : [],
    ticketLink: e.ticket_link || e.ticketLink || "#",
    image_url: e.image_url || null, is_free: e.is_free || false,
...
function OpenBadge({ open_now }) {
  if (open_now === null || open_now === undefined) return null;
  return (
    <span style={{ fontSize: 9, fontWeight: 700, padding: "2px 7px", borderRadius: 20, background: open_now ? "#22C55E22" : "#EF444422", color: open_now ? "#22C55E" : "#EF4444", fontFamily: "'JetBrains Mono', monospace", letterSpacing: 0.5 }}>
      {open_now ? "● Aberto" : "● Fechado"}
    </span>
  );
}
...
function PlaceDetail({ place, onClose, user, onToggleFav, isFav, handleShare, handleCheckin }) {
        ...
        <div style={{ padding: "0 20px 16px", borderBottom: "1px solid #1a1a1a" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              <button onClick={() => handleShare(place.name, window.location.href, `Olha esse lugar no vybe: ${place.name} — ${place.address}`)}
                style={{ display: "inline-flex", alignItems: "center", gap: 6, marginTop: 8, marginLeft: 8, background: "#1a1a1a", border: "1px solid #333", borderRadius: 20, padding: "5px 12px", cursor: "pointer" }}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#A78BFA" strokeWidth="2.5" strokeLinecap="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
                <span style={{ fontSize: 11, fontWeight: 700, color: "#A78BFA" }}>Compartilhar</span>
              </button>
            </div>
...
function EventDetail({ event, onClose, isFav, onToggleFav, handleShare }) {
  return (
...
function OnboardingScreen({ onFinish }) {
  ...
  const steps = [
    { icon: "🗺️", title: "Descubra o rolê", desc: "Veja bares, baladas e restaurantes no mapa em tempo real. Filtre por tipo, distância e vibe." },
    { icon: "🔴", title: "Ao Vivo de verdade", desc: "Usuários mandam updates de dentro dos lugares. Saiba se tem fila, qual DJ tá tocando e o clima agora." },
    { icon: "🎉", title: "Festas e eventos", desc: "Encontre eventos em SJC, veja lineup, preços e garanta seu ingresso. Tudo num só lugar." },
  ];
  const isLast = step === steps.length - 1;
  return (
    <div style={{ position: "fixed", inset: 0, background: "#080808", zIndex: 900, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 28px", fontFamily: "'Inter Tight', system-ui" }}>
      <div style={{ position: "absolute", top: "8%", left: "15%", width: 180, height: 180, borderRadius: "50%", background: "#A78BFA", opacity: 0.07, filter: "blur(60px)" }} />
      <div style={{ position: "absolute", bottom: "15%", right: "10%", width: 220, height: 220, borderRadius: "50%", background: "#F472B6", opacity: 0.07, filter: "blur(80px)" }} />
      <div style={{ fontSize: 80, marginBottom: 24 }}>{steps[step].icon}</div>
      <h2 style={{ margin: "0 0 12px", fontSize: 28, fontWeight: 900, color: "#f5f5f5", textAlign: "center", letterSpacing: -0.5 }}>{steps[step].title}</h2>
      <p style={{ margin: "0 0 40px", fontSize: 15, color: "#666", textAlign: "center", lineHeight: 1.6, maxWidth: 300 }}>{steps[step].desc}</p>
      <div style={{ display: "flex", gap: 8, marginBottom: 32 }}>
        {steps.map((_, i) => <div key={i} style={{ width: i === step ? 24 : 8, height: 8, borderRadius: 4, background: i === step ? "#A78BFA" : "#333", transition: "width 0.3s" }} />)}
      </div>
      <button onClick={() => { if (isLast) { localStorage.setItem("vybe_onboarded","1"); onFinish(); } else setStep(s => s + 1); }}
        style={{ width: "100%", maxWidth: 320, padding: 16, background: isLast ? "linear-gradient(90deg, #A78BFA, #F472B6)" : "#1a1a1a", border: isLast ? "none" : "1px solid #333", borderRadius: 16, color: isLast ? "#000" : "#f5f5f5", fontSize: 16, fontWeight: 800, cursor: "pointer" }}>
        {isLast ? "Explorar agora 🚀" : "Próximo →"}
      </button>
      {!isLast && <button onClick={() => { localStorage.setItem("vybe_onboarded","1"); onFinish(); }} style={{ marginTop: 12, background: "none", border: "none", color: "#444", fontSize: 13, cursor: "pointer" }}>Pular</button>}
    </div>
  );
}

// ─── BOMBANDO SECTION ─────────────────────────────────────────────────────────
function BombandoSection({ places, onSelect, userLocation }) {
  if (!places?.length) return null;
  return (
    <div style={{ padding: "12px 16px 0" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        <span style={{ fontSize: 16 }}>🔥</span>
        <span style={{ fontSize: 13, fontWeight: 800, color: "#f5f5f5", letterSpacing: -0.3 }}>Bombando agora</span>
        <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#EF4444", boxShadow: "0 0 6px #EF4444", animation: "pulse 2s infinite" }} />
      </div>
      <div style={{ display: "flex", gap: 10, overflowX: "auto", scrollbarWidth: "none", paddingBottom: 4 }}>
        {places.map(place => {
          const dist = userLocation ? fmtDist(haversineKm(userLocation.lat, userLocation.lng, place.lat, place.lng)) : "";
          return (
            <div key={place.id} onClick={() => onSelect(place)} style={{ flexShrink: 0, background: "#111", border: `1px solid ${place.color}44`, borderRadius: 14, padding: "10px 14px", cursor: "pointer", minWidth: 140 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                <span style={{ fontSize: 14 }}>{typeEmoji(place.type)}</span>
                <span style={{ fontSize: 8, fontWeight: 700, color: place.color, fontFamily: "'JetBrains Mono', monospace", textTransform: "uppercase" }}>{typeLabel(place.type)}</span>
              </div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#f5f5f5", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{place.name}</div>
              <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 4 }}>
                <span style={{ width: 5, height: 5, borderRadius: "50%", background: crowdColor(place.crowd) }} />
                <span style={{ fontSize: 10, color: crowdColor(place.crowd), fontFamily: "'JetBrains Mono', monospace", fontWeight: 700 }}>{place.crowd}%</span>
                {dist && <span style={{ fontSize: 10, color: "#555" }}>· {dist}</span>}
              </div>
            </div>
          );
        })}
...
export default function App() {
  ...
  const [showOnboarding, setShowOnboarding] = useState(() => !localStorage.getItem("vybe_onboarded"));
  const [filterDate, setFilterDate] = useState("Todos");
  const [filterOpenNow, setFilterOpenNow] = useState(false);
  const [filterFree, setFilterFree] = useState(false);
  const [bombandoPlaces, setBombandoPlaces] = useState([]);
  ...
  ususeEffect(() => {
    const fetchEvents = async () => {
      ...
      // Busca eventos ativos e futuros (com starts_at) + fallback para sem starts_at
      const { data, error } = await supabase
        .from("events")
        .select("*")
        .eq("status", "active")
  ...
  const handleShare = async (title, url, text) => {
    if (navigator.share) {
      ...
      try { await navigator.clipboard.writeText(url || window.location.href); alert("Link copiado!"); } catch {}
    }
  };
  ...
  const handleCheckin = async (placeId) => {
    ...
    if (!error) {
      await supabase.from("places").update({ checkins: supabase.rpc("increment") }).eq("id", placeId);
  ...
  const DATE_FILTERS = ["Todos", "Hoje", "Amanhã", "Fim de semana", "Este mês"];
  ...
  const filteredEvents = useMemo(() => {
    ...
    const nextSat = new Date(today); nextSat.setDate(today.getDate() + daysToSat);
    ...
    return events.filter(e => {
      ...
      if (filterFree && !e.is_free) return false;
      if (filterDate !== "Todos" && e.starts_at) {
        const evtDate = new Date(e.starts_at);
        const evtDay = new Date(evtDate.getFullYear(), evtDate.getMonth(), evtDate.getDate());
        if (filterDate === "Hoje" && evtDay.getTime() !== today.getTime()) return false;
        if (filterDate === "Amanhã" && evtDay.getTime() !== tomorrow.getTime()) return false;
        if (filterDate === "Fim de semana" && (evtDay < nextSat || evtDay > nextSun)) return false;
        if (filterDate === "Este mês" && evtDate > endOfMonth) return false;
      }
      return true;
    });
  }, [events, filterEventType, filterDate, filterFree, searchQuery, tab]);

  const filteredAreaPlaces = useMemo(() => {
    let places = areaPlaces;
    // Filter by search
    if (searchQuery && tab === "home") {
      const q = searchQuery.toLowerCase();
      places = dbPlaces.filter(p =>
    ...
    // Filter open now
    if (filterOpenNow) places = places.filter(p => p.open_now === true);
    // Sort by distance if location available
    if (userLocation) {
      places = [...places].sort((a, b) => {
        const da = haversineKm(userLocation.lat, userLocation.lng, a.lat, a.lng) || 999;
        const db = haversineKm(userLocation.lat, userLocation.lng, b.lat, b.lng) || 999;
        return da - db;
      });
    }
    return places;
  }, [areaPlaces, searchQuery, tab, filterOpenNow, userLocation, dbPlaces]);
        ...
        <div style={{ flex: 1, position: "relative", minHeight: 0, overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 200, padding: "20px 16px 12px", background: "linear-gradient(180deg, rgba(8,8,8,0.95) 60%, transparent)" }}>
            ...
            <div style={{ display: "flex", gap: 6, overflowX: "auto", scrollbarWidth: "none" }}>
              ...
              <button onClick={() => setFilterOpenNow(f => !f)} style={{ flexShrink: 0, padding: "5px 12px", borderRadius: 20, background: filterOpenNow ? "rgba(34,197,94,0.2)" : "rgba(0,0,0,0.8)", color: filterOpenNow ? "#22C55E" : "#aaa", border: filterOpenNow ? "1px solid #22C55E55" : "1px solid #333", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                ● Aberto
              </button>
            </div>
            {showSearch && (
          ...
          {bombandoPlaces.length > 0 && (
            <div style={{ position: "absolute", bottom: 260, left: 0, right: 0, zIndex: 200 }}>
              <BombandoSection places={bombandoPlaces} onSelect={setSelected} userLocation={userLocation} />
            </div>
          )}
