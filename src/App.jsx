...
ticketLink: "[https://sympla.com.br](https://sympla.com.br/)", emoji: "🎓",
},
{
...
<h3 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: "#f0f0f0" }}>{[place.name](http://place.name/)}</h3>
</div>
...
place_id: [place.id](http://place.id/),
user_name: "Você",
msg: newReport,
mood: newMood,
});
...
<h2 style={{ margin: "6px 0 2px", fontSize: 22, fontWeight: 800, color: "#f5f5f5" }}>{[place.name](http://place.name/)}</h2>
...
<h3 style={{ margin: "8px 0 4px", fontSize: 18, fontWeight: 900, color: "#fff", lineHeight: 1.2 }}>{[event.name](http://event.name/)}</h3>
</div>
...
<span style={{ fontSize: 10, fontWeight: 700, padding: "2px 10px", borderRadius: 20, background: "rgba(0,0,0,0.3)", color: "#fff", textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: "'JetBrains Mono', ui-monospace, monospace" }}>{event.type}</span>
<h2 style={{ margin: "8px 0 4px", fontSize: 24, fontWeight: 900, color: "#fff" }}>{[event.name](http://event.name/)}</h2>
...
[link.id](http://link.id/) = "leaflet-css";
link.rel = "stylesheet";
...
const link = document.createElement("link");
[link.id](http://link.id/) = "vybe-fonts";
link.rel = "stylesheet";
...
[style.id](http://style.id/) = "vybe-map-style";
style.innerHTML =         `.leaflet-container { background: #080808 !important; }         .vybe-marker { background: transparent !important; border: none !important; }         .vybe-pin-wrap { display:flex; flex-direction:column; align-items:center; cursor:pointer; }         .vybe-pin { width:42px; height:42px; border-radius:50% 50% 50% 0; transform:rotate(-45deg); display:flex; align-items:center; justify-content:center; box-shadow:0 6px 18px rgba(0,0,0,0.5); border:2px solid rgba(255,255,255,0.65); }         .vybe-pin span { transform:rotate(45deg); font-size:18px; line-height:1; }         .vybe-pin-dot { width:7px; height:7px; border-radius:50%; margin-top:2px; opacity:0.75; }         @keyframes userPulse { 0%,100%{transform:scale(1);opacity:0.3} 50%{transform:scale(2);opacity:0} }`      ;
...
const placeMap = {};
dbPlaces.forEach(p => { placeMap[[p.id](http://p.id/)] = p; });
...
id: [e.id](http://e.id/), name: [e.name](http://e.name/), type: e.type, date: e.date, time: e.time,
...
setFavPlaces(prev => prev.some(p => [p.id](http://p.id/) === [place.id](http://place.id/)) ? prev.filter(p => [p.id](http://p.id/) !== [place.id](http://place.id/)) : [...prev, place]);

const toggleFavEvent = (event) =>
setFavEvents(prev => prev.some(e => [e.id](http://e.id/) === [event.id](http://event.id/)) ? prev.filter(e => [e.id](http://e.id/) !== [event.id](http://event.id/)) : [...prev, event]);
